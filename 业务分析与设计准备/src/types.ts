...
export type AuditStatus = "草稿" | "待审核" | "已通过" | "已驳回" | "已打绩效" | "已结算" | "已撤销"
...
// ===== 执行链配置（药厂可决定任务执行是否经过「工作组」） =====

/** 链节点标识（取值与 BizRole 四类一致，但 BizRole 本身不改动） */
export type ChainNodeId = "药厂" | "服务提供商" | "工作组" | "服务专员"

/** 生效链级别：工作组启用 = 四级链，停用 = 三级链 */
export type ChainLevel = "四级链" | "三级链"
...
// ===== 服务商下层品种授权（对象跟随链路：四级链=工作组，三级链=服务专员） =====
...
export type BizRole = "药厂" | "服务提供商" | "工作组" | "服务专员"
...
export interface VisitRecord {
  ...
  performanceStatus: VisitPerformanceStatus
...
/** 任务量：专员执行记录；「已完成」=已审核，可选入结算。粒度 = 单品种 + 单地区。三级链任务直挂专员，workGroup 为空 */
export interface WorkloadAssign {
  id: string
  /** 所属工作组；三级链（停用工作组）任务直挂专员时为空 */
  workGroup?: string
  specialist: string
  variety: string
  region: string
  ...
  /** 部分结算累计的工作量；达到 workload 时整行结算完毕 */
  settledQty?: number
}
...
export interface ReportFile {
  ...
  /** 关联服务项目（报告类任务结算计价依据） */
  itemName?: string
  /** 报告归属服务大类（分析报告服务 / 问卷调研与分析服务） */
  category?: string
  /** 报告归属服务月份，格式 YYYY-MM；发起结算按此归集 */
  serviceMonth?: string
  /** 已选入的结算单号；未选入为空 */
  settledBillNo?: string
  /** 重新上传版本号，首次为 1 */
...
export interface SettlementBill {
  ...
  /** 确认后实际 − 申请 */
  adjustAmount: number
  /** 发起时勾选的任务量及本次结算量（部分结算回退时按此返还占用） */
  workloadSettledQty?: { workloadId: string; qty: number }[]
...
export interface Task {
  ...
  /** 创建时锁定的执行链快照；后续修改链配置不得影响既有任务 */
  chainSnapshot: ExecutionChain
  /** 预算推荐总金额（仅提示） */
...
export interface MenuItem {
  /** 目录用语义 id（如 "group-admin"）；页面菜单用 "menu-<pageId>" */
  id: string
  name: string
  /** group=目录，page=页面菜单 */
  type: MenuType
  ...
  /** type=page 时必填，关联已有页面 */
  pageId?: PageId
  /** 一级项图标，MENU_ICONS 的 key */
  iconKey?: string
  /** 一级项所属分区标签（""=无标签分区），仅种子使用，V1 不编辑 */
  section?: string
  /** 同级排序，父级内/一级各自连续编号 */
  sort: number
  enabled: boolean
...
export type DashboardMetricIcon = "clipboard" | "percent" | "progress" | "wallet" | "alert" | "shield" | "file-search" | "building" | "users" | "check" | "camera" | "coins" | "user-check" | "clock" | "briefcase" | "receipt"
...
export type ApprovalBizType = "代表准入" | "代表授权" | "代表解冻" | "服务商准入" | "服务商复审" | "服务商解冻" | "高风险例外" | "项目验收" | "结算例外"
...
// ── 服务商准入（服务商提交 / 药厂合规审核 双角色最小闭环）──────────────────
// 旧 Vendor 类型仍供代表备案等页面使用；本节为准入页的最小读取模型。

export type VendorAccessStatus = "草稿" | "待提交" | "已通过" | "已驳回"

export interface VendorAccessHistory {
  id: string
