      ...
      <div style={{ flex: 1, overflow: "auto", padding: 16 }}>
        ...
        {pageData.length === 0 ? (
          ...
          <div style={{ background: "#FFFFFF", border: "1px solid var(--color-border)", borderRadius: 8, overflow: "auto" }}>
            <table className="tm-table" style={{ width: "100%", borderCollapse: "collapse", minWidth: 1040 }}>
              <thead>
                <tr>
                  {["企业名称", "类型", "统一社会信用代码", "主企业码", "首位管理员", "租户状态", "创建时间", "操作"].map((h) => (
                    <th key={h}>{h}</th>
                  ))}
                </tr>
              </thead>
...
const emptyForm = (type: TenantType): WizardForm => ({
  ...
  pharmaCategory: PHARMA_CATEGORIES[0], mahHolderFlag: true, licenseNo: "", licenseValidTo: "", complianceContact: "",
  bizOwner: "", complianceOwner: "", qualificationNo: "", qualificationValidTo: "",
  adminName: "", adminPhone: "", adminDepartment: "",
});
...
function TenantWizard({
  ...
  const steps = ["选择企业类型", "填写企业资料", "指定首位管理员"];
  ...
  const validateStep = (s: number): boolean => {
    ...
    if (s === 1) {
      ...
      if (form.type === "pharma" && !form.licenseAttachment) errs.licenseAttachment = "请上传营业执照或证明材料（模拟附件）";
      if (form.type === "pharma") {
        if (!form.licenseNo.trim()) errs.licenseNo = "请输入许可证编号";
        if (!form.complianceContact.trim()) errs.complianceContact = "请输入合规联系人";
      } else {
  ...
  const buildProfile = (): TenantSubjectProfile => ({
    ...
    ...(form.type === "pharma"
      ? {
          pharmaCategory: form.pharmaCategory,
          mahHolderFlag: form.mahHolderFlag,
          licenseNo: form.licenseNo.trim(),
          licenseValidTo: form.licenseValidTo,
          complianceContact: form.complianceContact.trim(),
        }
      : {
  ...
  const submit = async () => {
    ...
    try {
      const result = createTenant({ type: form.type, profile: buildProfile(), admin: buildAdmin(), actor });
    ...
    <>
      ...
      {step === 1 && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", columnGap: 16, rowGap: 16 }}>
          ...
          <Field
            id="wf-license"
            label={form.type === "pharma" ? "营业执照或证明材料" : "营业执照或证明材料（选填）"}
            required={form.type === "pharma"}
            error={errors.licenseAttachment}
            hint={form.type === "pharma" ? "原型使用模拟附件，不产生真实文件" : "选填；原型使用模拟附件，不产生真实文件"}
          >
            <div style={{ display: "flex", gap: 8 }}>
              <input {...inputProps("wf-license", errors.licenseAttachment)} value={form.licenseAttachment} readOnly placeholder="未选择材料" style={{ ...fieldInput, background: "#F9FAFB", ...(errors.licenseAttachment ? { borderColor: "#DC2626" } : {}) }} />
          ...
          {form.type === "pharma" ? (
            <>
              <Field id="wf-pharmaCategory" label="主体分类" required>
              ...
              <Field id="wf-licenseNo" label="相关许可证编号" required error={errors.licenseNo}>
                <input {...inputProps("wf-licenseNo", errors.licenseNo)} value={form.licenseNo} onChange={(e) => set({ licenseNo: e.target.value })} placeholder="如：苏药监械生产许 20260115" />
              </Field>
              ...
              <Field id="wf-complianceContact" label="合规联系人" required error={errors.complianceContact}>
                <input {...inputProps("wf-complianceContact", errors.complianceContact)} value={form.complianceContact} onChange={(e) => set({ complianceContact: e.target.value })} />
              </Field>
            </>
          ) : (
            <>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", columnGap: 16, rowGap: 16 }}>
            ...
            <Field id="wf-adminPhone" label="管理员手机号" required error={errors.adminPhone} hint="仅代表联系方式控制权，不作为租户主键；首位登录后自动激活">
              <input {...inputProps("wf-adminPhone", errors.adminPhone)} className="tm-field font-mono-nums" value={form.adminPhone} onChange={(e) => set({ adminPhone: e.target.value.replace(/\D/g, "") })} maxLength={11} placeholder="11 位手机号" style={{ ...fieldInput, ...(errors.adminPhone ? { borderColor: "#DC2626" } : {}) }} />
            </Field>
          ...
          <dl className="tm-confirm-grid">
            ...
            <dt>首位管理员</dt><dd>{form.adminName || "—"} · <span className="font-mono-nums">{form.adminPhone || "—"}</span> · {form.adminDepartment || "—"}</dd>
          </dl>
          <div className="tm-note tm-note--brand" role="note">
            <ShieldCheck size={14} aria-hidden />
            <span>确认创建后，系统将生成<b>不可修改的主企业码</b>，并创建首位管理员<b>待激活</b>账号；租户初始状态为「待激活」，首位管理员首次登录验证码通过后自动激活。</span>
          </div>
        </div>
      )}
    </>
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "space-between", gap: 8, marginTop: 16, flexWrap: "wrap" }}>
          ...
          >
            ...
            {busy ? "正在创建租户并生成企业码…" : step < 2 ? "下一步" : "确认创建"}
          </button>
        </div>
      </Modal>
...
function TenantDetailDrawer({
  ...
  const [adminForm, setAdminForm] = useState({ name: "", phone: "", department: "", reason: "" });
              ...
              <dl className="tm-confirm-grid">
                ...
                {tenant.type === "pharma" ? (
                  <>
                    ...
                    <dt>合规联系人</dt><dd>{p.complianceContact || "—"}</dd>
                  </>
                ) : (
                  <>
            ...
            <section>
              ...
              {tenant.status === "pending_activation" && (
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 10, flexWrap: "wrap" }}>
                  <button type="button" className="tm-ghost-btn" disabled={busy !== null} onClick={() => { setAdminForm({ name: "", phone: "", department: "", reason: "" }); setAdminErr({}); setAdminChangeOpen(true) }}>
              ...
              <table className="tm-table" style={{ width: "100%", borderCollapse: "collapse", minWidth: 640 }}>
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      {tenant.status === "pending_activation" ? (
                        <button type="button" className="tm-row-btn" onClick={() => { setAdminForm({ name: "", phone: "", department: "", reason: "" }); setAdminErr({}); setAdminChangeOpen(true) }}>
                          变更
                        </button>
                      ) : (
        ...
        <Modal open title="变更首位管理员" width={460} onClose={() => { if (busy === null) setAdminChangeOpen(false) }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <Field id="ca-phone" label="新管理员手机号" required error={adminErr.phone}>
              <input {...inputProps("ca-phone", adminErr.phone)} className="tm-field font-mono-nums" value={adminForm.phone} onChange={(e) => setAdminForm((f) => ({ ...f, phone: e.target.value.replace(/\D/g, "") }))} maxLength={11} style={{ ...fieldInput, ...(adminErr.phone ? { borderColor: "#DC2626" } : {}) }} />
            </Field>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
              ...
              <button type="button" className="tm-primary-btn" disabled={busy !== null} onClick={() => {
                ...
                void run("change-admin", () => {
                  const r = changeInitialAdmin(tenant.id, { name: adminForm.name.trim(), phone: adminForm.phone, department: adminForm.department.trim(), roleLabel: tenant.initialAdmin.roleLabel }, adminForm.reason.trim(), actor);
