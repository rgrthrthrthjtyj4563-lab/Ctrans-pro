...
export function defaultSettlementPeriods(startDate: string, endDate: string): Omit<SettlementPeriod, 'id'>[] {
...
export function validateSettlementPeriods(
  ...
  for (const p of sorted) {
    ...
    if (p.startDate < startDate || p.endDate > endDate) return '结算周期必须完全落在推广期内';
  }
  if (sorted[0].startDate !== startDate) return '结算周期必须从推广期开始日连续覆盖，不得有空档';
  if (sorted[sorted.length - 1].endDate !== endDate) return '结算周期必须覆盖至推广期结束日，不得有空档';
...
export function periodCoversMonth(period: SettlementPeriod, serviceMonth: string): boolean {
...
export function findPeriodForMonth(task: Task, serviceMonth: string): SettlementPeriod | undefined {
...
export function assertSamePriceBook(
  ...
  if (missing) {
    return { ok: false, error: `品种「${missing.v.tradeName}」没有有效价目表，无法创建任务` };
  }
...
export function nearestTier(amount: number, unitPrice: number): {
  ...
  if (unitPrice <= 0) {
    return { qty: 0, amount: 0, rounded: false, rawQty: 0, reason: '单价须大于 0' };
  }
...
export function roundingToastText(log: Pick<RoundingLog, 'beforeAmount' | 'beforeQty' | 'unitPrice' | 'afterAmount' | 'afterQty' | 'reason'>): string {
  return `调整前金额 ${formatCNY(log.beforeAmount)}、数量 ${Number.isInteger(log.beforeQty) ? log.beforeQty : log.beforeQty.toFixed(2)}；适用单价 ${formatCNY(log.unitPrice)}；调整后金额 ${formatCNY(log.afterAmount)}、数量 ${log.afterQty}。${log.reason}`;
}
...
export function recommendedBudget(params: {
  ...
  const matched = plans.filter((p) => {
    ...
    if (Number(startDate.slice(0, 4)) !== p.year && Number(endDate.slice(0, 4)) !== p.year) return false;
  ...
  if (!matched.length) return { amount: 0, configured: false, detail: '未配置预算' };
  let amount = 0;
  matched.forEach((p) => {
    months.forEach((ym) => {
...
export function confirmedSettledAmount(task: Task): number {
  return validBills(task).filter((b) => b.confirmed).reduce((s, b) => s + b.finalAmount, 0);
}
...
export function suggestedOf(book: PriceBook, category: string, name: string): { unitPrice: number; unit: string } | undefined {
  const rule = book.rules.find((r) => r.category === category && r.name === name);
  if (!rule) return undefined;
  return { unitPrice: rule.amount, unit: rule.unit };
}

export function serviceKey(it: Pick<ServiceItem, 'variety' | 'region' | 'category' | 'name'>): string {
