          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 16 }}>
            <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 8, overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', borderBottom: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', gap: 8 }}>
                <LayoutDashboard size={15} style={{ color: '#176B5B' }} />
                <span style={{ fontSize: 14, fontWeight: 600 }}>将看到的菜单与按钮</span>
                <span style={{ marginLeft: 'auto', fontSize: 12, color: '#9CA3AF' }}>{pageRows.length} 个页面</span>
              </div>
                  ...
                  {pageRows.map((p, idx) => (
                    <tr key={p.id} style={{ background: idx % 2 === 0 ? '#fff' : '#FAFAFA' }}>
                      <td style={tdStyle}>
                        ...
                        <div style={{ fontSize: 12, color: '#9CA3AF' }}>{p.module}</div>
                      </td>
                      <td style={tdStyle}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 8, padding: 16 }}>
                <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 10 }}>预览上下文</div>
                ...
                <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#C77A16' }}>
                  <Lock size={13} /> 预览会话禁止写操作与导出
                </div>
              </div>

              <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 8, overflow: 'hidden' }}>
                <div style={{ padding: '12px 16px', borderBottom: '1px solid #E5E7EB', fontSize: 14, fontWeight: 600 }}>
                  敏感字段展示
                </div>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <tbody>
...
function KV({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, padding: '6px 0', fontSize: 13, borderBottom: '1px solid #F3F4F6' }}>
